...
func newClientWithOAuth(ctx context.Context, cfg *app.ServerserviceOptions, logger *logrus.Logger) (*sservice.Client, error) {
	fmt.Println("newClientWithOAuth")
...
// BatchBomsUpload will attempt to write multiple boms to database.
func (s *Serverservice) BatchBomsUpload(ctx context.Context, boms []sservice.Bom) (*sservice.ServerResponse, error) {
	return s.client.BillOfMaterialsBatchUpload(ctx, boms)
}

// GetAOCMacAddr will return the bom info object by the aoc mac address.
func (s *Serverservice) GetAOCMacAddr(ctx context.Context, macAddr string) (*sservice.Bom, *sservice.ServerResponse, error) {
